...
def extract_device_from(row, network):
    ...
    device = Device(network=network if cells[0] == '\xa0' else cells[0][:-1], name=cells[1], macAddress=cells[2], ipAddress=cells[3])
...
def get_devices(url):
    ...
    table = soup.find(string='Devices currently connected to your Plusnet Hub:').find_parent('table')        
    devices = []
    device = None
...
def init(url):
    ...
    def render(draw, width, height):
        ...
        for index, group in groupby(devices, key=by_name):
            ...
            y_offset += 11
            for device in group:
                draw.text((10, y_offset), text=device.ipAddress, fill='white', font=default)
                legacy.text(draw, (width - 8, y_offset), '\1' if device.network == 'Ethernet' else '\0', fill='white', font=CUSTOM_BITMAP_FONT)
                y_offset += 11
